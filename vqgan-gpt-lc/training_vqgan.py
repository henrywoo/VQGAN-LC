...
class ImageNetDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return [image_ids, image, clip_image, label]
...
class FFHQDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return [image_ids, image, clip_image, 0]
...
def main(args):
    ...
    if os.path.exists(os.path.join(args.output_dir, 'vqgan_checkpoint-last.pth')):
        try:
            ...
            files = [os.path.join(args.output_dir, f) for f in os.listdir(args.output_dir) if f != "vqgan_checkpoint-last.pth" and os.path.splitext(f)[1] == ".pth"]
            files.sort(key=lambda x: os.path.getmtime(x), reverse=True)
            print(files)
            ckpt = torch.load(files[0])
            model_without_ddp.load_state_dict(ckpt["state_dict"], strict=True)
            #ckpt = torch.load(os.path.join(args.output_dir, 'vqgan_checkpoint-last.pth'), map_location="cpu")
